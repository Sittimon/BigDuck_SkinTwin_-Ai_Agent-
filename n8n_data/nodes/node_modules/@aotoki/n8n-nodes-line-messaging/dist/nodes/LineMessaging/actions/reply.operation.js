...
exports.properties = [
    (0, operationProperties_1.getReplyTokenProperty)(),
    (0, messageProperties_1.getMessageProperties)(['reply']),
];
...
async function execute(items) {
    ...
    for (let i = 0; i < items.length; i++) {
        try {
            ...
            const replyToken = this.getNodeParameter('replyToken', i, '');
            if (!replyToken) {
                throw new n8n_workflow_1.NodeOperationError(this.getNode(), 'Reply token is required. Get it from the webhook trigger event.');
            }
            ...
            const responseData = await GenericFunctions_1.apiRequest.call(this, 'POST', '/message/reply', {
                replyToken,
                messages,
            });
            returnData.push({
